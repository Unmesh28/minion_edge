...
def process_input(data1) :
    ...
    X = []
    #X.append(data[0])
    X.append(clean_voltage(data[0]))  
    X.append(clean_current(data[1]))
    X.append(clean_realPower(data[2]))
    X.append(clean_powerFactor(data[3]))
    X.append(data[4])
    X.append(clean_reactive_power(data[5]))
    X.append(clean_frequency(data[6]))
    print(X)
    sys.stdout.flush()
    get_model_result(X)
